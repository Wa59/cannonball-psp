...
void Config::load(const std::string &filename)
{
    ...
    menu.enabled           = pt_config.get("menu.enabled",   0);
    ...
    video.fps        = pt_config.get("video.fps",                0); // Default is 60 fps
    video.fps_count  = pt_config.get("video.fps_counter",        0); // FPS Counter
    video.widescreen = 0; // Enable Widescreen Mode
    ...
    controls.steer_speed   = pt_config.get("controls.steerspeed", 9);
    controls.pedal_speed   = pt_config.get("controls.pedalspeed", 9);
    ...
    engine.dip_traffic   = pt_config.get("engine.traffic", 0);
